...
class BottleNeck(tf.keras.layers.Layer):
    def __init__(self, in_size, exp_size, out_size, s, is_se_existing, NL, k, alpha=1.0):
        super(BottleNeck, self).__init__()
        self.stride = s
        self.in_size = in_size
        self.out_size = depth(out_size * alpha)
        self.exp_size = depth(exp_size * alpha)
        self.is_se_existing = is_se_existing
        self.NL = NL
        self.conv1 = tf.keras.layers.Conv2D(filters=self.exp_size,
                                            kernel_size=(1, 1),
                                            strides=1,
                                            padding="same")
        ...
        self.se = SEBlock(input_channels=self.exp_size)
        self.conv2 = tf.keras.layers.Conv2D(filters=self.out_size,
                                            kernel_size=(1, 1),
                                            strides=1,
                                            padding="same")
